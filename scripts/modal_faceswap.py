...
image = (
 ...
 .pip_install(
 "comfy-cli", "huggingface_hub[hf_transfer]", "hf_transfer",
 "httpx", "fastapi", "uvicorn", "Pillow", "pyyaml",
 ...
 # ComfyUI_FaceAnalysis (insightface-based)
...
def download_all_models(civitai_token=""):
 ...
 tasks = [
 # DarkBeast (Flux2 face swap model)
 (curl_file, (
...
def build_faceswap_workflow(face_filename: str, body_filename: str, seed: int) -> dict:
 ...
 "28": {
 "inputs": {
 "face": True, "hair": False, "body": False, "clothes": False,
 "accessories": False, "background": False, "confidence": 0.2,
 "detail_method": "VITMatte(local)", "detail_erode": 6, "detail_dilate": 6,
 "black_point": 0.01, "white_point": 0.99, "process_detail": True,
 "device": "cuda", "max_megapixels": 2,
 "images": ["295", 0]
 },
 ...
 "class_type": "SetLatentNoiseMask", "_meta": {"title": "Set Latent Noise Mask"}
 },
 "58": {
 "inputs": {"model": ["59", 0], "processor": ["60", 0], "image": ["245", 0]},
 "class_type": "FaceParse(FaceParsing)", "_meta": {"title": "FaceParse(FaceParsing)"}
 },
 "59": {
 "inputs": {"device": "cuda"},
 ...
 "62": {
 "inputs": {
 "background": False, "skin": False, "nose": True, "eye_g": False,
 "r_eye": True, "l_eye": True, "r_brow": True, "l_brow": True,
 "r_ear": False, "l_ear": False, "mouth": True, "u_lip": True,
 "l_lip": True, "hair": False, "hat": False, "ear_r": False,
 "neck_l": False, "neck": False, "cloth": False,
 "result": ["58", 1]
 },
 ...
 "66": {
 "inputs": {
 "expand": 10, "incremental_expandrate": 0, "tapered_corners": True,
 "flip_input": False, "blur_radius": 5, "lerp_alpha": 1,
 "decay_factor": 1, "fill_holes": False, "mask": ["62", 0]
 },
 "class_type": "GrowMaskWithBlur", "_meta": {"title": "Grow Mask With Blur"}
 },
 "82": {
 "inputs": {
 "force_resize_width": 0, "force_resize_height": 0,
 "image": ["245", 0], "mask": ["84", 0]
 },
 "class_type": "Cut By Mask", "_meta": {"title": "Cut By Mask"}
 },
 "84": {
 "inputs": {"mask": ["66", 0]},
 "class_type": "MaskToImage", "_meta": {"title": "Convert Mask to Image"}
 },
 "86": {
 "inputs": {
 "x": 0, "y": 0, "resize_source": False,
 "destination": ["245", 0], "source": ["251", 0], "mask": ["66", 0]
 },
 "class_type": "ImageCompositeMasked", "_meta": {"title": "ImageCompositeMasked"}
 },
 "242": {
 "inputs": {
 "padding": 300, "padding_percent": 0, "index": 0,
 "analysis_models": ["247", 0], "image": ["296", 0]
 },
 "class_type": "FaceBoundingBox", "_meta": {"title": "Face Bounding Box"}
 },
 "245": {
 "inputs": {
 "width": 1000, "height": 1000, "interpolation": "lanczos",
 "method": "keep proportion", "condition": "always",
 "multiple_of": 0, "image": ["242", 0]
 },
 "class_type": "ImageResize+", "_meta": {"title": "Image Resize"}
 },
 "247": {
 "inputs": {"library": "insightface", "provider": "CUDA"},
 "class_type": "FaceAnalysisModels", "_meta": {"title": "Face Analysis Models"}
 },
 "250": {
 "inputs": {
 "padding": 300, "padding_percent": 0, "index": 0,
 "analysis_models": ["253", 0], "image": ["295", 0]
 },
 "class_type": "FaceBoundingBox", "_meta": {"title": "Face Bounding Box"}
 },
 "251": {
 ...
 "255": {
 "inputs": {
 "width": ["242", 3], "height": ["242", 4], "interpolation": "lanczos",
 "method": "keep proportion", "condition": "always",
 "multiple_of": 0, "image": ["86", 0]
 },
 "class_type": "ImageResize+", "_meta": {"title": "Image Resize"}
 },
 "256": {
 "inputs": {
 "x": ["242", 1], "y": ["242", 2], "resize_source": False,
 "destination": ["296", 0], "source": ["255", 0], "mask": ["66", 0]
 },
 "class_type": "ImageCompositeMasked", "_meta": {"title": "ImageCompositeMasked"}
 },
 "259": {
 ...
 "class_type": "KSamplerSelect", "_meta": {"title": "KSamplerSelect"}
 },
 "273": {
 "inputs": {
 "cfg": 1, "model": ["279", 0],
 "positive": ["286", 0], "negative": ["283", 0]
 },
 "class_type": "CFGGuider", "_meta": {"title": "CFGGuider"}
 },
 "274": {
 ...
 "class_type": "ReferenceLatent", "_meta": {"title": "ReferenceLatent"}
 },
 "276": {
 ...
 "scale_to_length": 1280, "background_color": "#000000",
 "image": ["306", 0]
 },
 ...
 "class_type": "DifferentialDiffusion", "_meta": {"title": "Differential Diffusion"}
 },
 "280": {
 "inputs": {"image": ["298", 0]},
 ...
 "281": {
 "inputs": {
 "width": ["280", 0], "height": ["280", 1], "batch_size": 1
 },
 "class_type": "EmptyFlux2LatentImage", "_meta": {"title": "Empty Flux 2 Latent"}
 },
 "282": {
 "inputs": {
 "noise": ["274", 0], "guider": ["273", 0],
 "sampler": ["272", 0], "sigmas": ["289", 0],
 "latent_image": ["281", 0]
 },
 ...
 "image1": ["287", 0], "image2": ["295", 0]
 },
 "class_type": "ImageConcanate", "_meta": {"title": "Image Concatenate (Swwan)"}
 },
 "286": {
 ...
 "289": {
 "inputs": {
 "steps": 6, "width": ["280", 0], "height": ["280", 1]
 },
 "class_type": "Flux2Scheduler", "_meta": {"title": "Flux2Scheduler"}
 },
 "290": {
 "inputs": {
 ...
 "297": {
 "inputs": {
 "seed": seed + 1, "steps": 5, "cfg": 1,
 "sampler_name": "euler", "scheduler": "ddim_uniform",
 "denoise": 0.3, "model": ["291", 0],
 "positive": ["299", 0], "negative": ["292", 0],
 "latent_image": ["38", 0]
 },
 ...
 "class_type": "LoadImage", "_meta": {"title": "face"}
 },
 "307": {
 "inputs": {"image": body_filename},
 "class_type": "LoadImage", "_meta": {"title": "body"}
 },
 }
