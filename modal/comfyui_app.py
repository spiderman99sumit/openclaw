import modal, subprocess, os, time, json, urllib.request
...
image = (
    ...
    .pip_install("aiohttp", "requests", "Pillow", "numpy", "safetensors")
    .run_commands(
...
@app.function(gpu="A10G", timeout=900, volumes={"/storage": vol})
def generate_image(prompt: str, negative_prompt: str, job_id: str = "test", image_id: str = "001"):
    ...
    try:
        ...
        wf = {
            ...
            "9":{"class_type":"SaveImage","inputs":{"filename_prefix":f"{job_id}_{image_id}","images":["8",0]}}
        }
        ...
        for _ in range(180):
            ...
            if pid in hist and "9" in hist[pid].get("outputs", {}):
                ...
                if imgs:
                    src = f"/root/ComfyUI/output/{imgs[0]['filename']}"
